...
export const loader = (async ({ params }) => {
	const repo = await queryClient.ensureQueryData(repoQueryOptions(params.team!, params.repo!));
...
export const handle = {
	breadcrumb: (data: Awaited<ReturnType<typeof loader>>) => (
		<div>
			<Link to={generatePath('/t/:team', { team: data.repo.team.slug })} className="hover:underline">
				{data.repo.team.name}
			</Link>
			<span> / </span>
			<Link
...
export const Component = () => {
	...
	const { data: repo } = useSuspenseQuery(repoQueryOptions(params.team!, params.repo!));
...
const PipelineList = ({ repo }: { repo: components['schemas']['RepoResponse'] }) => {
	...
	return (
		<div className="border border-black/10 divide-y divide-black/10 mt-4">
			{data.items.length === 0 && <div className="text-center p-4"> -비어있음- </div>}
			{data.items.map((x) => (
...
const PipelineItem = ({ pipeline }: { pipeline: components['schemas']['PipelineResponse'] }) => {
	...
	return (
		<div className="py-4 px-6 gap-4 flex items-center">
			...
			<Link to={pipelineLink} className="font-mono font-bold hover:underline opacity-60">
