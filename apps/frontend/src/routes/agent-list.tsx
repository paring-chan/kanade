...
const AgentItem = ({ agent }: { agent: components['schemas']['AgentResponse'] }) => {
	return (
		<div className="border -mt-px border-gray-300 px-4 py-2 flex items-center gap-4">
			...
			<div className="text-sm opacity-40">
				{agent.status === 'offline' ? '오프라인' : agent.status === 'busy' ? '작업 처리 중' : agent.status === 'idle' ? '온라인' : '알 수 없음'}
			</div>
				...
				<AlertDialog.Portal>
					...
					>
						...
						<div className={dialog.titleArea()}>
							<AlertDialog.Title className={dialog.title()}>에이전트 삭제</AlertDialog.Title>
						</div>
...
const CreateAgentDialog = () => {
	...
	const createdHandle = useMemo(() => AlertDialog.createHandle<{ name: string; token: string }>(), []);

	const form = useForm({
		defaultValues: { name: '', teamId: '' } as type.infer<typeof createAgentSchema>,
		...
		<>
			<Dialog.Root handle={dialogHandle}>
				...
				<Dialog.Portal>
					...
					>
						<div className={dialog.titleArea()}>
							<Dialog.Title className={dialog.title()}>에이전트 생성</Dialog.Title>
						</div>
						<div className={dialog.content()}>
							<form.Field
								...
								children={(field) => (
									...
									>
										<Field.Label className={formField.label()}>에이전트 이름</Field.Label>
										<Field.Control className={input()} value={field.state.value} onValueChange={field.handleChange} onBlur={field.handleBlur} placeholder="Example Team" />
							...
							<form.Field
								...
								children={(field) => (
									<Field.Root className={formField.root()}>
										<Field.Label className={formField.label()}>스코프</Field.Label>
										<ScopeSelector
			...
			<AlertDialog.Root handle={createdHandle}>
				{({ payload }) => (
					<Dialog.Portal>
						...
						>
							<div className={dialog.titleArea()}>
								<Dialog.Title className={dialog.title()}>에이전트 생성됨</Dialog.Title>
								<Dialog.Description className={dialog.description()}>아래 토큰으로 에이전트를 설정해주세요. 토큰은 한번만 표시됩니다.</Dialog.Description>
							</div>
							...
							<div>
								<Field.Root className={formField.root()}>
									...
									<Field.Control className={input()} value={payload?.token} readOnly />
								</Field.Root>
							</div>

							<div className={dialog.actionsArea()}>
								<Dialog.Close className={button({ style: 'normal' })}>닫기</Dialog.Close>
							</div>
						</Dialog.Popup>
					</Dialog.Portal>
				)}
...
const ScopeSelector = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
	...
	return (
		...
		>
			<Combobox.InputGroup className={combobox.inputGroup()}>
				<Combobox.Input placeholder="스코프 선택" className={combobox.input()} />
				<Combobox.Clear>
					<LuX />
				</Combobox.Clear>
