...
export const loader = (async ({ params }) => {
	const pipeline = await queryClient.ensureQueryData(pipelineQueryOptions(params.pipeline!));
	const repo = await queryClient.ensureQueryData(repoByIdQueryOptions(pipeline.repoId));

	return { pipeline, repo };
}) satisfies LoaderFunction;

const Breadcrumb = ({ pipeline: { id: pipelineId }, repo }: Awaited<ReturnType<typeof loader>>) => {
	...
	const icon = (() => {
		switch (pipeline.status) {
			...
			case 'running':
				return <LuLoaderCircle className="size-4 animate-spin text-yellow-400" />;
			case 'failed':
	...
	return (
		<div className="flex gap-1 items-center">
			<Link to={generatePath('/t/:team', { team: repo.team.slug })} className="hover:underline">
				{repo.team.name}
			</Link>
			<span> / </span>
			<Link
...
export const Component = () => {
	...
	const { data: pipeline } = useSuspenseQuery(pipelineQueryOptions(params.pipeline!));
	...
	return (
		<div className="flex grow">
            ...
            파이프라인 정보
					...
					<button className="flex items-center cursor-pointer px-3 hover:bg-black/10 transition-colors border-b  border-black">작업 목록</button>
					{/*<button className="flex items-center cursor-pointer px-3 hover:bg-black/10 transition-colors">
            DAG
          </button>*/}
				</div>
...
const JobTree = ({ pipelineId, onSelect }: { pipelineId: string; onSelect: (jobId: string) => void }) => {
...
const JobItem = ({ job, onSelect }: { job: components['schemas']['PipelineJobResponse']; onSelect: () => void }) => {
	const icon = (() => {
		switch (job.status) {
			case 'cancelled':
				return <LuBan className="size-4 text-gray-400" />;
	...
	return (
		<button className="px-4 py-1 flex items-center gap-2 transition-colors text-left cursor-pointer hover:bg-black/5" onClick={onSelect}>
