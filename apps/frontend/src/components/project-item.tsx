...
export const ProjectItem = ({ repo }: { repo: components['schemas']['RepoResponse'] }) => {
	const upstream = new URL(repo.upstream_url);
		...
		<div className="p-4 flex items-center gap-4 bg-pink-50 border border-black/10 -ml-px -mt-px">
			<div className="max-w-64 grow min-w-0">
				<div className="text-lg leading-3.5 flex whitespace-nowrap truncate pb-1">
					<Link className="hover:underline" to={generatePath('/t/:team', { team: repo.team.slug })}>
						{repo.team.slug}
					</Link>
				...
				<a href={repo.upstream_url} className="text-xs opacity-40 hover:underline truncate" target="_blank" rel="noopener">
					{upstream.host}
					{upstream.pathname}
				</a>
			</div>
