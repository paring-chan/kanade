...
export const CreateProjectDialog = ({ defaultTeamId }: { defaultTeamId?: string }) => {
	...
	return (
		...
		>
			<Dialog.Trigger className={button({ style: 'outlined' })}>생성</Dialog.Trigger>
			...
			<Dialog.Portal>
				...
				>
					<div className={dialog.titleArea()}>
						<Dialog.Title className={dialog.title()}>프로젝트 생성</Dialog.Title>
						<Dialog.Description className={dialog.description()}>
							선택한 포지의 저장소에 대해 CI를 활성화합니다. 저장소 관리 권한이 있는 경우에만 목록에 표시됩니다.
						</Dialog.Description>
					</div>

					<form.Subscribe selector={(state) => state.values} children={(values) => <pre>{JSON.stringify(values, null, 2)}</pre>} />
					...
					<div className={dialog.content()}>
						<form.Field
							...
							children={(field) => (
								...
								>
									<Field.Label className={formField.label()}>프로젝트 이름</Field.Label>
									<Field.Control className={input()} value={field.state.value} onValueChange={field.handleChange} onBlur={field.handleBlur} placeholder="Project Mizuki" />
									<div className={formField.helperArea()}>
										<Field.Description className={formField.description()}>최대 20자</Field.Description>
										<Field.Error className={formField.error()} match={!field.state.meta.isValid}>
						...
						<form.Field
							...
							children={(field) => (
								<Field.Root
									...
									touched={field.state.meta.isTouched}
								>
									<Field.Label className={formField.label()}>프로젝트 슬러그</Field.Label>
									<Field.Control className={input()} value={field.state.value} onValueChange={field.handleChange} onBlur={field.handleBlur} placeholder="project-mizuki" />
									<div className={formField.helperArea()}>
										<Field.Description className={formField.description()}>최대 20자, 알파벳, 숫자, -, _만 사용 가능</Field.Description>
										<Field.Error className={formField.error()} match={!field.state.meta.isValid}>
											{field.state.meta.errors.join(',')}
										</Field.Error>
									</div>
						...
						<form.Field
							...
							children={(forgeField) => (
								<>
									<Field.Root className={formField.root()}>
										<Field.Label className={formField.label()}>포지</Field.Label>
										<ForgeSelector
										...
										<div className={formField.helperArea()}>
											<Field.Error className={formField.error()} match={!forgeField.state.meta.isValid}>
									...
									<form.Field
										...
										children={(repoField) => (
											<Field.Root className={formField.root()}>
												<Field.Label className={formField.label()}>저장소</Field.Label>
												<RepositorySelector
												...
												<div className={formField.helperArea()}>
													<Field.Error className={formField.error()} match={!repoField.state.meta.isValid}>
					...
					<div className={dialog.actionsArea()}>
						<Dialog.Close className={button({ style: 'outlined' })}>취소</Dialog.Close>
...
const TeamSelector = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
...
const ForgeSelector = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
...
const RepositorySelector = ({ forgeId, value, onChange }: { value: string; onChange: (value: string) => void; forgeId: string }) => {
	...
	return (
		...
		>
			<Combobox.InputGroup className={combobox.inputGroup()}>
				<Combobox.Input placeholder="저장소 선택" className={combobox.input()} />
				<Combobox.Clear>
					<LucideX />
				</Combobox.Clear>
				<Combobox.Trigger className={combobox.trigger()}>
